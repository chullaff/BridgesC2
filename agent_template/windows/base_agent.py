...
# import platform
# import requests
# from commands import sysinfo, ping, download, upload
...
#     #     try:
#     #         res = requests.post(f"{SERVER_URL}/api/agents", json={
...
#         try:
#             res = requests.get(f"{SERVER_URL}/api/tasks/{self.agent_id}")
#             if res.status_code == 200:
#                 return res.json()
#         except Exception as e:
...
#         try:
#             res = requests.post(f"{SERVER_URL}/api/results/", json={
#                 "agent_id": self.agent_id,
#                 "task_id": task_id,
